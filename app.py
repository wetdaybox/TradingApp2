...
What It Does:
- Automatically installs/upgrades required packages (when run locally).
- Fetches free, up-to-date AAPL data from Yahoo Finance.
- Checks if "Adj Close" exists; if not, uses "Close" for prices.
- Computes a 50-day Simple Moving Average (SMA) as a trend signal.
- Generates a binary trading signal (1 if price > SMA, else 0).
- Simulates an aggressive leveraged strategy with dynamic, risk-based position sizing.
- Plots an interactive chart showing the stock price with the SMA and the cumulative return.
- Displays a clear trade recommendation (action, number of shares, leverage, stop-loss, and take-profit).
- Saves the simulation results to a CSV file so you can see past performance.

DISCLAIMER: This system is experimental and uses leverage. No system is foolproof.
Use it with extreme caution and only for educational purposes.
"""
...
# Import the libraries (tkinter is imported for possible error alerts, but not used in Streamlit)
...
# Set up logging to a file for troubleshooting
...
def fetch_stock_data(stock_symbol, start_date, end_date):
    ...
    It will first try to use 'Adj Close'. If that is not available, it uses 'Close'.
    """
...
def generate_signal(df, sma_window=50):
    ...
    Generate a binary signal:
      - 1 if the current price is greater than the 50-day SMA (trend up)
      - 0 if not.
...
def dynamic_position_size(current_price, stop_loss_pct, portfolio_value, risk_pct=0.01):
    """
    Determine how many shares to buy so that only risk_pct of the portfolio is risked on a trade.
    For a long trade, the risk per share is current_price * stop_loss_pct.
    """
...
def simulate_leveraged_cumulative_return(df, leverage=5):
    """
    Simulate the cumulative return for the strategy.
...
def save_results(df, filename="trading_results.csv"):
    ...
    to a CSV file for future review.
    """
    result = pd.DataFrame({
...
def plot_results(df, stock_symbol, start_date, end_date):
    ...
    Create a two-panel plot:
      - Upper: Stock price and the 50-day SMA.
      - Lower: Cumulative leveraged return.
...
st.markdown("### About")
st.markdown("""
This application is designed to run autonomously using free historical data.
